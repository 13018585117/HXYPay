...
public class NotactivationFragment extends BaseFragment {

    private ExpandableListView mListView;
    private NotActivationListAdapter mAdapter;
    private List<FriendsRes.Info1> mData = new ArrayList<FriendsRes.Info1>();
            ...
            public void onResponse(FriendsRes response, int id) {
                ...
                if (response != null) {
                    if (response.getCode().equals("0000")) {
                        if(response.getData().getVIP()!=null) {
                            direct = response.getData().getVIP().getDirect();
                            indirect = response.getData().getVIP().getIndirect();
                        }
                        FragmentExpandableListAdapter vipFragmentExpandableListAdapter = new FragmentExpandableListAdapter(getActivity(), direct, indirect);
                        mListView.setAdapter(vipFragmentExpandableListAdapter);
