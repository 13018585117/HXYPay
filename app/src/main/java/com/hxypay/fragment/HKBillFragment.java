...
import com.hxypay.R;
import com.hxypay.adapter.HKBillListAdapter;
...
public class HKBillFragment extends BaseFragment {

    private ListView mListView;
    private HKBillListAdapter mAdapter;
    private List<HKBillRes.Info1> mData = new ArrayList<HKBillRes.Info1>();
...
//                }, 2000);
    ...
    public void getHKBills(String pageNum) {
        OkHttpUtils.post()
                .url(Constants.HK_BILLS)
                .headers(PublicParam.getParam())
                .params(Params.hkBill(pageNum))
                .build().execute(new GenericsCallback<HKBillRes>(new JsonGenericsSerializator()) {
            @Override
            ...
            @Override
            public void onResponse(HKBillRes response, int id) {
                mPullToRefreshLayout.finishRefresh();
                dismissDialog();
                if (response != null) {
